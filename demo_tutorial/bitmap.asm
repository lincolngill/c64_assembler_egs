...
//
.var pic = LoadBinary("car.koa", BF_KOALA)
//.var pic = LoadBinary("ww.koa", BF_KOALA)
            *=$6000 "Pic"           // $6000 is in VIC bank 1
//bitmap:           .fill pic.getSize(), pic.get(i)     // All bytes in file
bitmap:     .fill pic.getBitmapSize(), pic.getBitmap(i)
colour:     .fill pic.getColorRamSize(), pic.getColorRam(i)
...
.print "colour RAM addr: $"+toHexString(colour)
.print "colour size: "+pic.getColorRamSize()+" Hex: $"+toHexString(pic.getColorRamSize())
.print "bkgnd colour: $"+toHexString(pic.getBackgroundColor())

			* = $C000 "Bitmap loader"   // Tape buffer
start:	
            lda #$00
            tax
...
copyloop:
            .for (var i=0; i<4; i++) {
                lda colour+i*$0100,x      // copy colours to colour RAM
                sta $D800+i*$0100,x
            }
            dex
            bne copyloop
